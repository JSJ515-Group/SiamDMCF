...
class ChannelAggregationFFN(nn.Module):
    ...
    def forward(self, x):
        # proj 1
        x = self.fc1(x)     # 1x1 conv
        x = self.dwconv(x)  # 3x3 DWConv
        x = self.act(x)     # GELU
        x = self.drop(x)    # dropout
        # proj 2
        x = self.feat_decompose(x)
        x = self.fc2(x)     # conv 1x1
        x = self.drop(x)
        return x
...
class MultiOrderGatedAggregation(nn.Module):
    ...
    def __init__(self,
        ...
        self.embed_dims = embed_dims
        self.attn_force_fp32 = attn_force_fp32  # False
    ...
    def feat_decompose(self, x):
        x = self.proj_1(x)      # 1x1conv
        # x_d: [B, C, H, W] -> [B, C, 1, 1]
        x_d = F.adaptive_avg_pool2d(x, output_size=1)  # GAP
        x = x + self.sigma(x - x_d)     # θ_m * Subtract(x,x_d) ,残差
        x = self.act_value(x)  # SiLU
        return x    # Z
    ...
    def forward(self, x):
        shortcut = x.clone()
        # proj 1x1
        x = self.feat_decompose(x)  # 返回Z
        # gating and value branch
        g = self.gate(x)    # 1x1conv
        v = self.value(x)   # 多阶DWConv卷积处理
        # aggregation(TRUE)
        if not self.attn_force_fp32:
            x = self.proj_2(self.act_gate(g) * self.act_gate(v))    # conv1x1{SiLU(g)*SiLU(v1)}
        else:
...
class MOGF(nn.Module):
    ...
    def __init__(self,
        ...
        self.attn = MultiOrderGatedAggregation(
            ...
            attn_act_type=attn_act_type,    # SiLU
...
if __name__ == '__main__':
    ...
    MOGF = MOGF(64)
    output = MOGF(input)
    print(' MOGF_input_size:', input.size())
    print(' MOGF_output_size:', output.size())

    output = MOGF(input)
    print('MOGF input_size:', input.size())
    print('MOGF output_size:', output.size())
