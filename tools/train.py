...
def build_opt_lr(model, current_epoch=0):
    ...
    trainable_params += [{'params': model.dmcf_head.parameters(),
                          'lr': cfg.TRAIN.BASE_LR}]
...
def main():
    ...
    # create model
    model = ModelBuilder()
    print("[DEBUG] Model has dmcf_head?", hasattr(model, 'dmcf_head'))
    print("[DEBUG] Model attributes:", [attr for attr in dir(model) if not attr.startswith('_')])
